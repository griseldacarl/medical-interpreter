...
function blankTerm(term: string): string {
  const isCJKorArabic = /[\u4e00-\u9fff\u0600-\u06ff]/.test(term)
  const numBlanks = isCJKorArabic
    ? (Math.random() < 0.5 ? 1 : 2)
    : Math.ceil(Math.random() * 3)

  const positions = shuffle(Array.from({ length: term.length }, (_, i) => i))
  const blankSet = new Set(positions.slice(0, Math.min(numBlanks, term.length - 1)))

  return term.split('').map((c, i) => blankSet.has(i) ? '_' : c).join('')
}
...
export function FillInBlank({ terms, sourceLang, onComplete }: FillInBlankProps) {
  const startTime = useMemo(() => Date.now(), [])

  const questions = useMemo(() => {
    const count = Math.min(QUESTION_COUNT, terms.length)
  ...
  if (!current || questions.length === 0) {
    return (
      <div className="flex items-center justify-center py-16">
        <p className="text-slate-500">Not enough terms for this game. Try selecting a different system.</p>
      </div>
    )
  }
  ...
  function handleNext() {
    if (isLast) {
      onComplete({ gameType: 'fill-in-blank', score, total: questions.length, timeMs: Date.now() - startTime })
      return
    }
    setIndex(i => i + 1)
    setSelected(null)
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-8 rounded-lg bg-white p-6 text-center shadow-lg sm:p-10">
        <span className="mb-2 block text-sm font-medium text-slate-500">
          {current.translations.en}
        </span>
        <p className="font-mono text-3xl font-bold tracking-widest text-slate-800 sm:text-5xl">
          {blanked}
        </p>
      </div>
