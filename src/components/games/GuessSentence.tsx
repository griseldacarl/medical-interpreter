...
function partialSentence(sentence: string): { fragment: string; isBeginning: boolean } {
  const isBeginning = Math.random() < 0.5
  const revealRatio = Math.random() < 0.5 ? 0.4 : 0.25
  const revealLen = Math.max(Math.floor(sentence.length * revealRatio), 3)

  if (isBeginning) {
    return { fragment: sentence.slice(0, revealLen) + '...', isBeginning: true }
  }
  return { fragment: '...' + sentence.slice(-revealLen), isBeginning: false }
}

interface GuessSentenceProps {
  terms: TranslationEntry[]
  sourceLang: LanguageCode
  onComplete: (results: GameResults) => void
}

export function GuessSentence({ terms, sourceLang, onComplete }: GuessSentenceProps) {
  ...
  const questions = useMemo(() => {
    const count = Math.min(QUESTION_COUNT, interviewTerms.length)
  ...
  function handleNext() {
    if (isLast) {
      onComplete({ gameType: 'guess-sentence', score, total: questions.length, timeMs: Date.now() - startTime })
      return
    }
    setIndex(i => i + 1)
    setSelected(null)
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-8 rounded-lg bg-white p-6 shadow-lg sm:p-8">
        ...
        <span className="mb-2 block text-sm font-medium text-slate-500">
          {sentenceInfo.isBeginning ? 'Beginning of sentence:' : 'Ending of sentence:'}
        </span>
        <p className="text-xl font-bold text-slate-800 sm:text-2xl">
          {sentenceInfo.fragment}
        </p>
      </div>
